...
export default async (
    ...
    ****************************************/
    ...
    // 2.4. Deploy MIX (w/ BasketManager addr)
    ...
    // 2.4.2. Deploy MIX
    await deployer.deploy(c_Mix, { from: default_ });
    const d_mUSD = await c_Mix.deployed();
    // Initialize MIX implementation to avoid external party doing so
    await d_mUSD.initialize("", "", DEAD_ADDRESS, DEAD_ADDRESS, DEAD_ADDRESS);
    // Deploy MIX proxy
    await deployer.deploy(c_MixProxy);
    const d_mUSDProxy = await c_MixProxy.deployed();
  ...
  ****************************************/
    ...
    console.log(`[MIX]: '${d_mUSDProxy.address}'`);
    console.log(`[MIX impl]: '${d_mUSD.address}'`);
