 ...
 *          bAsset tokens using ETH from the Kyber platform and mint mIx tokens from mStable.
 */
contract MintWithKyber is AbstractBuyAndMint, ReentrancyGuard {
     ...
     */
    constructor(address _kyberNetworkProxy, address[] memory _mIxs)
        public
        ...
        returns (uint256 mIxQtyMinted)
    {
        ...
        mIxQtyMinted = _buyAndMint(_srcBasset, _destMix, msg.value, _msgSender());
    }

    // @override
    function buyAndMintGivenMix(
        address _srcBasset,
        address _destMix,
        ...
        returns (uint256 mIxQtyMinted)
    {
        ...
        mIxQtyMinted = _buyAndMint(_srcBasset, _destMix, amountInETH, _msgSender());
        ...
        returns (uint256 mIxQtyMinted)
    {
        ...
        mIxQtyMinted = _buyAndMintMulti(_srcBassets, _ethAmount, _destMix, _msgSender());
    }

    /**
     * @dev Buy bAssets with ETH and Mint mIx from mStable. Send mIxs to the user.
     * @param _srcBasset Source bAsset to buy from Kyber
     * @param _destMix mIx to mint from mStable
     * @param _ethAmount Amount of ETH to user to buy bAssets
     * @param _recipient Recipient of the mStable tokens
     * @return mIxQtyMinted Returns the total quantity of mIxs minted
     */
        ...
        returns (uint256 mIxQtyMinted)
    {
        ...
        mIxQtyMinted = IMix(_destMix).mintTo(address(_srcBasset), bAssetQtyMinted, _recipient);
    }

    /**
     * @dev Buy multiple bAssets using corrosponding ETH amount from Kyber and mint mIxs
     *      using these bAssets.
     * @param _srcBassets Array of bAssets to buy from Kyber
     * @param _ethAmounts Array of eth amount to use corrosponding bAssets
     * @param _destMix mIx address to mint
     * @param _recipient Recipient of the mStable tokens
     * @return mIxQtyMinted Returns the total quantity of mIxs minted
     */
        ...
        returns (uint256 mIxQtyMinted)
    {
        ...
        mIxQtyMinted = IMix(_destMix).mintMulti(_srcBassets, bAssetsQtyMinted, _recipient);
    }
