...
import { IUniswapV2Router02 } from "../../mix/liquidator/IUniswapV2Router02.sol";
import { ICurveBildPool } from "../../mix/liquidator/ICurveBildPool.sol";
...
contract SaveWrapper {
    ...
    constructor(
        ...
        vault = _vault;
        require(_mIx != address(0), "Invalid mIx address");
        mIx = _mIx;
        ...
        curve = ICurveBildPool(_curveAddress);

        IERC20(_mIx).safeApprove(save, uint256(-1));
        ...
        for(uint256 i = 0; i < _bAssets.length; i++ ) {
            IERC20(_bAssets[i]).safeApprove(_mIx, uint256(-1));
        }
    }


    /**
     * @dev 0. Simply saves an mIx and then into the vault
     * @param _amount Units of mIx to deposit to savings
     */
    function saveAndStake(uint256 _amount) external {
        IERC20(mIx).transferFrom(msg.sender, address(this), _amount);
        uint256 credits = ISavingsContractV2(save).depositSavings(_amount);
        IBoostedSavingsVault(vault).stake(msg.sender, credits);
    }

    /**
     * @dev 1. Mints an mIx and then deposits to SAVE
    ...
    function saveViaMint(address _bAsset, uint256 _amt, bool _stake) external {
        ...
        // 2. Mint
        IMix mIx_ = IMix(mIx);
        uint256 mixsMinted = mIx_.mint(_bAsset, _amt);
        // 3. Mint imUSD and optionally stake in vault
        _saveAndStake(mixsMinted, _stake);
    }
    ...
    /**
     * @dev Gets estimated mIx output from a Curve trade
     */
    ...
    /**
     * @dev Gets estimated mIx output from a WETH > bAsset > mIx trade
     */
