    ...
    InitializableModule2
{
        ...
        initializer
    {
        ...
        require(address(_underlying) != address(0), "mIx address is zero");
    ...
    ****************************************/
        ...
        returns (uint256 creditsIssued)
    {
        ...
        IERC20 mIx = underlying;
        if(_collectInterest){
            ISavingsManager(_savingsManager()).collectAndDistributeInterest(address(mIx));
        }

        // Transfer tokens from sender to here
        require(mIx.transferFrom(msg.sender, address(this), _underlying), "Must receive tokens");
    ...
    ****************************************/
     ...
     * @return mixReturned  Units of underlying mIx paid out
     */
        ...
        returns (uint256 creditsBurned)
    {
        ...
        (uint256 credits, uint256 mixReturned) = _redeem(_underlying, false);
        require(mixReturned == _underlying, "Invalid output");
    ...
    function _redeem(uint256 _amt, bool _isCreditAmt)
        internal
        returns (uint256 creditsBurned, uint256 mixReturned)
    {
    ...
    ****************************************/
    ...
    function _poke(CachedData memory _data, bool _ignoreCadence) internal {
        ...
        if(address(connector_) != address(0)){
            ...
            //     If there is not enough mIx in the connector, then deposit
            ...
            //     Else withdraw, if there is too much mIx in the connector
    ...
    ****************************************/
    ...
    /**
     * @dev Converts mix amount into credits based on exchange rate
     *               c = (mix / exchangeRate) + 1
     */
    ...
    /**
     * @dev Converts credit amount into mix based on exchange rate
