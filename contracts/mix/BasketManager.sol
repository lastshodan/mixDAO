...
/**
 * @title   BasketManager
 * @notice  Manages the Basket composition for a particular mIx. Feeds all required
 *          basket data to the mIx and is responsible for keeping accurate records.
    ...
    InitializableReentrancyGuard
{
    ...
    // mIx linked to the manager (const)
    address public mIx;
     ...
     * @param _mIx           Address of the mIx whose Basket to manage
    ...
    /**
     * @dev Verifies that the caller is governed mIx
     */
    modifier onlyMix() {
        require(mIx == msg.sender, "Must be called by mIx");
        _;
    }
    ...
    ****************************************/
    ...
    /**
     * @dev Called by only mIx, and only when the basket is healthy, to add units to
    ...
    /**
     * @dev Called by mIx after redeeming tokens. Simply reduce the balance in the vault
     * @param _bAssetIndex      Index of the bAsset
    ...
    /**
     * @dev Called by mIx to calculate how much interest has been generated in the basket
     *      and withdraw it. Cycles through the connected platforms to check the balances.
     * @return interestCollected   Total amount of interest collected, in mIx terms
    ...
    ****************************************/
    ...
    /**
     * @dev Get basket details for `MixStructs.Basket`
     ...
     * @param _isMint    Is this swap actually a mint? i.e. output == address(mIx)
     * @return props     Struct of all relevant Forge information
     */
