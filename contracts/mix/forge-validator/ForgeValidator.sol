...
import { IForgeValidator, MixStructs } from "./IForgeValidator.sol";
...
contract ForgeValidator is IForgeValidator {
    ...
    ****************************************/
        ...
        returns (bool isValid, string memory reason)
    {
        ...
        // How much mIx is this _bAssetQuantity worth?
        uint256 mintAmountInMix = _bAssetQuantity.mulRatioTruncate(_bAsset.ratio);
        // How much of this bAsset do we have in the vault, in terms of mIx?
        uint256 newBalanceInMix = _bAsset.vaultBalance.mulRatioTruncate(_bAsset.ratio).add(mintAmountInMix);
        // What is the max weight of this bAsset in the basket?
        uint256 maxWeightInUnits = (_totalVault.add(mintAmountInMix)).mulTruncate(_bAsset.maxWeight);

        if(newBalanceInMix > maxWeightInUnits) {
        ...
        returns (bool isValid, string memory reason)
    {
        ...
        for(uint256 j = 0; j < bAssetCount; j++){
            ...
            // How much mIx is this bassetquantity worth?
            uint256 mintAmountInMix = _bAssetQuantities[j].mulRatioTruncate(b.ratio);
            // How much of this bAsset do we have in the vault, in terms of mIx?
            newBalances[j] = b.vaultBalance.mulRatioTruncate(b.ratio).add(mintAmountInMix);

            newTotalVault = newTotalVault.add(mintAmountInMix);
        }
    ...
    ****************************************/
        ...
        returns (bool isValid, string memory reason, uint256 output, bool applySwapFee)
    {
        ...
        // How much mIx is this _bAssetQuantity worth?
        uint256 inputAmountInMix = _quantity.mulRatioTruncate(_inputBasset.ratio);

        // 1. Determine output bAsset valid
        //  - Enough units in the bank
        uint256 outputAmount = inputAmountInMix.divRatioPrecisely(_outputBasset.ratio);
        ...
        applySwapFee = true;
        uint256 outputBalanceMix = _outputBasset.vaultBalance.mulRatioTruncate(_outputBasset.ratio);
        uint256 outputMaxWeightUnits = _totalVault.mulTruncate(_outputBasset.maxWeight);
        if(outputBalanceMix > outputMaxWeightUnits) {
            applySwapFee = false;
        }

        // 2. Calculate input bAsset valid - If incoming basket goes above weight, then fail
        // How much of this bAsset do we have in the vault, in terms of mIx?
        uint256 newInputBalanceInMix = _inputBasset.vaultBalance.mulRatioTruncate(_inputBasset.ratio).add(inputAmountInMix);
        // What is the max weight of this bAsset in the basket?
        uint256 inputMaxWeightInUnits = _totalVault.mulTruncate(_inputBasset.maxWeight);
        if(newInputBalanceInMix > inputMaxWeightInUnits) {
    ...
    ****************************************/
        ...
        returns (bool, string memory, bool)
    {
        ...
        for(uint256 i = 0; i < idxCount; i++){
            ...
            // Calculate ratioed redemption amount in mIx terms
        ...
        returns (bool, string memory, uint256[] memory)
    {
        ...
        if(_mIxQuantity > totalBassetVault) return (false, "Not enough liquidity", redeemQuantities);
        ...
        for(uint256 i = 0; i < len; i++) {
            ...
            uint256 ratioedProportionalBasset = _mIxQuantity.mulTruncate(percentageOfVault);
