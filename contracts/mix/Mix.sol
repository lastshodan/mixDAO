...
import { MixStructs } from "./shared/MixStructs.sol";

// Libs
import { StableMath } from "../shared/StableMath.sol";
import { MixHelpers } from "./shared/MixHelpers.sol";
    ...
    InitializableReentrancyGuard
{
    ...
    // Forging Events
    event Minted(address indexed minter, address recipient, uint256 mIxQuantity, address bAsset, uint256 bAssetQuantity);
    event MintedMulti(address indexed minter, address recipient, uint256 mIxQuantity, address[] bAssets, uint256[] bAssetQuantities);
    event Swapped(address indexed swapper, address input, address output, uint256 outputAmount, address recipient);
    event Redeemed(address indexed redeemer, address recipient, uint256 mIxQuantity, address[] bAssets, uint256[] bAssetQuantities);
    event RedeemedMix(address indexed redeemer, address recipient, uint256 mIxQuantity);
    ...
    ****************************************/
     ...
     * @return mixMinted   Number of newly minted mIxs
     */
    function mint(
        address _bAsset,
     ...
     * @param _bAsset         Address of the bAsset to mint
     * @param _bAssetQuantity Quantity in bAsset units
     * @param _recipient receipient of the newly minted mIx tokens
     * @return mixMinted   Number of newly minted mIxs
     */
    function mintTo(
        address _bAsset,
    ...
    /**
     * @dev Mint with multiple bAssets, at a 1:1 ratio to mIx. This contract
     ...
     * @param _recipient        Address to receive the newly minted mIx tokens
     * @return mixMinted     Number of newly minted mIxs
     */
    function mintMulti(
    ...
    ****************************************/
        ...
        returns (uint256 mixMinted)
    {
        ...
        uint256 mIxQuantity = 0;
        uint256[] memory receivedQty = new uint256[](len);

        // Transfer the Bassets to the integrator, update storage and calc MixQ
        for(uint256 i = 0; i < len; i++){
            ...
            if(bAssetQuantity > 0){
                ...
                mIxQuantity = mIxQuantity.add(ratioedDeposit);
            }
        }
        require(mIxQuantity > 0, "No mix quantity to mint");
        ...
        // Mint the Mix
        _mint(_recipient, mIxQuantity);
        emit MintedMulti(msg.sender, _recipient, mIxQuantity, _bAssets, _bAssetQuantities);
        ...
        returns (uint256 quantityDeposited, uint256 ratioedDeposit)
    {
        ...
        (uint256 transferred, uint256 cacheBal) = MixHelpers.transferReturnBalance(msg.sender, _integrator, _bAsset, _quantity);
    ...
    ****************************************/
    ...
    /**
     * @dev Simply swaps one bAsset for another bAsset or this mIx at a 1:1 ratio.
     ...
     * @param _output       Asset to receive - either a bAsset or mIx(this)
        ...
        returns (uint256 output)
    {
        ...
        // 1. If the output is this mIx, just mint
     ...
     * @param _input        bAsset to deposit
     * @param _output       Asset to receive - bAsset or mIx(this)
     * @param _quantity     Units of input bAsset to swap
        ...
        returns (bool, string memory, uint256 output)
    {
        ...
        // 2.1. If output is mIx(this), then calculate a simple mint
        if(isMint){
            // Validate mint
            ...
            // Simply cast the quantity to mIx
    ...
    ****************************************/
     ...
     *      relative mIx quantity from the sender. Sender also incurs a small mIx fee, if any.
     * @param _bAsset           Address of the bAsset to redeem
     * @param _bAssetQuantity   Units of the bAsset to redeem
     * @return mixMinted     Relative number of mIx units burned to pay for the bAssets
     */
    function redeem(
        address _bAsset,
        uint256 _bAssetQuantity
     ...
     *      relative Mix quantity from the sender. Sender also incurs a small fee, if any.
     * @param _bAsset           Address of the bAsset to redeem
     * @param _bAssetQuantity   Units of the bAsset to redeem
     ...
     *      relative Mix quantity from the sender. Sender also incurs a small fee on the outgoing asset.
     ...
     * balance levels and desired mIx quantity. Burns the mIx as payment.
     * @param _mIxQuantity   Quantity of mIx to redeem
    ...
    function redeemMix(
        ...
        nonReentrant
    {
        _redeemMix(_mIxQuantity, _recipient);
    }
    ...
    ****************************************/
    ...
    /** @dev Redeem mIx for one or more bAssets */
    function _redeemTo(
        address[] memory _bAssets,
        uint256[] memory _bAssetQuantities,
        address _recipient
        ...
        returns (uint256 mixRedeemed)
    {
        ...
        // Calc total redeemed mIx quantity
        for(uint256 i = 0; i < bAssetCount; i++){
            uint256 bAssetQuantity = _bAssetQuantities[i];
            if(bAssetQuantity > 0){
                // Calc equivalent mIx amount
                uint256 ratioedBasset = bAssetQuantity.mulRatioTruncateCeil(props.bAssets[i].ratio);
                mIxQuantity = mIxQuantity.add(ratioedBasset);
            }
        }
        require(mIxQuantity > 0, "Must redeem some bAssets");

        // Redemption has fee? Fetch the rate
        uint256 fee = applyFee ? swapFee : 0;

        // Apply fees, burn mIx and return bAsset to recipient
        _settleRedemption(
        ...
        emit Redeemed(msg.sender, _recipient, mIxQuantity, _bAssets, _bAssetQuantities);
        return mIxQuantity;
    }


    /** @dev Redeem mIx for a multiple bAssets */
    function _redeemMix(
        uint256 _mIxQuantity,
        address _recipient
    )
    ...
    {
        require(_recipient != address(0), "Must be a valid recipient");
        require(_mIxQuantity > 0, "Invalid redemption quantity");
        ...
        // Ensure payout is related to the collateralised mIx quantity
        uint256 collateralisedMixQuantity = _mIxQuantity.mulTruncate(colRatio);

        // Calculate redemption quantities
        (bool redemptionValid, string memory reason, uint256[] memory bAssetQuantities) =
            forgeValidator.calculateRedemptionMulti(collateralisedMixQuantity, props.bAssets);
        require(redemptionValid, reason);
        ...
        emit RedeemedMix(msg.sender, _recipient, _mIxQuantity);
    }

    /**
     * @param _recipient        Recipient of the bAssets
     * @param _mIxQuantity   Total amount of mIx to burn from sender
     ...
     * burning sufficient mIx before withdrawing all tokens
     */
    function _settleRedemption(
        RedemptionSettlement memory args
    ) internal {
        // 1.0. Burn the full amount of Mix
        _burn(msg.sender, args.mIxQuantity);
    ...
    ****************************************/
    ...
    /**
     * @dev Gets the supply and cache details for the mIx, taking into account the surplus
    ...
    ****************************************/
    ...
    /**
      ...
      * @param _cacheSize Maximum percent of total mIx supply to hold for each bAsset
      */
    ...
    /**
      * @dev Set the ecosystem fee for redeeming a mIx
    ...
    /**
      * @dev Gets the address of the BasketManager for this mIx
    ...
    ****************************************/
    ...
    /**
     * @dev Converts recently accrued swap fees into mIx
     * @return swapFeesGained        Equivalent amount of mIx units that have been generated
     * @return newSupply             New total mIx supply
     */
        ...
        returns (uint256 swapFeesGained, uint256 newSupply)
    {
        ...
        if(surplus > 1){
            ...
            // mint new mIx to savings manager
     ...
     *      amount of mIx and sending it over to the SavingsManager.
