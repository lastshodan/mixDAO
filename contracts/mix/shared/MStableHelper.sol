...
import { MixStructs } from "./MixStructs.sol";
 ...
 *          DATE:    2020-06-18
 */
contract MStableHelper is IMStableHelper, MixStructs {
    ...
    ****************************************/
    ...
    {
        require(_mIx != address(0), "Invalid mIx");
        // Get the data
        IBasketManager basketManager = IBasketManager(
            IMix(_mIx).getBasketManager()
        );
        Basket memory basket = basketManager.getBasket();
        uint256 totalSupply = IMix(_mIx).totalSupply();
    ...
    {
        Data memory data = _getData(_mIx, _input, _output);
        if(!data.isValid) {
        ...
        if (data.isMint) {
            ...
            uint256 fee = applyFee ? data.mIx.swapFee() : 0;
    ...
    {
        // Convert the `mIxQuantity` (input) into bAsset units
        IBasketManager basketManager = IBasketManager(
            IMix(_mIx).getBasketManager()
        );
        Basset memory bAsset = basketManager.getBasset(_outputBasset);
        uint256 bAssetQuantity = _mIxQuantity.divRatioPrecisely(
            bAsset.ratio
        );
        ...
        ) = _getRedeemValidity(_mIx, bAssets, quantities);
    ...
    ****************************************/
     ...
     * @param _save SAVE contract address
     * @param _mIxUnits Amount of mIx to redeem from SAVE
        ...
        )
    {
        ...
        uint256 credits = _mIxUnits.divPrecisely(rate);
    ...
    ****************************************/
    ...
    function _getData(address _mIx, address _input, address _output) internal view returns (Data memory) {
        bool isMint = _output == _mIx;
        IMix mIx = IMix(_mIx);
        IBasketManager basketManager = IBasketManager(
            mIx.getBasketManager()
        );
        (bool isValid, string memory reason, ) = mIx
            .getSwapOutput(_input, _output, 1);
        uint256 totalSupply = mIx.totalSupply();
        ...
        )
    {
        ...
        uint256 fee = applyFee ? mIx.swapFee() : 0;
