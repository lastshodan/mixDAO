...
const c_MixProxy = artifacts.require("MixProxy");
...
// Mix
const c_Mix = artifacts.require("Mix");
...
export class MixMachine {
    ...
    /**
     * @dev Deploys an mIx with default parameters, modelled on original mUSD
     * @return Interface will all deployed information
     */
    public async deployMix(enableUSDTFee = false, useOldAave = false): Promise<MixDetails> {
        const md: MixDetails = {};
        ...
        *************************************** */
        ...
        // 2.4.2. Deploy mUSD
        // Deploy implementation
        const d_mUSD = await c_Mix.new();
        await d_mUSD.initialize("", "", DEAD_ADDRESS, DEAD_ADDRESS, DEAD_ADDRESS);
        // Deploy proxy
        const d_mUSDProxy = await c_MixProxy.new();
        // Initialize proxy
        ...
        md.mIx = await c_Mix.at(d_mUSDProxy.address);
    ...
    /**
     * @dev Deploy a Mix via the Manager then:
     *      1. Mint with optimal weightings
     */
    public async deployMixAndSeedBasket(
        enableUSDTFee = false,
        initialSupply = 100,
    ): Promise<MixDetails> {
        const mixDetails = await this.deployMix(enableUSDTFee);

        // Mint initialSupply with shared weightings
        const basketDetails = await this.getBassetsInMix(mixDetails);
        ...
        // Approve bAssets
        await Promise.all(
            mixDetails.bAssets.map((b, i) =>
                b.approve(mixDetails.mIx.address, mintAmounts[i].muln(2), {
    ...
    public async getBassetsInMix(mixDetails: MixDetails): Promise<Basset[]> {
        const response = await mixDetails.basketManager.getBassets();
        ...
        const integratorAddresses = await Promise.all(
            bArrays.map((b, i) => mixDetails.basketManager.integrations(i)),
        );
    ...
    public async getBasketComposition(mixDetails: MixDetails): Promise<BasketComposition> {
        // raw bAsset data
        const bAssets = await this.getBassetsInMix(mixDetails);
        const basket = await mixDetails.basketManager.getBasket();
        // total supply of mIx
        const supply = await mixDetails.mIx.totalSupply();
     ...
     * @param mIx Mix that gets permission to spend
     * @param fullMixUnits Whole number or fraction to approve
     * @param sender Set if needed, else fall back to default
     * @param inputIsBaseUnits Override the scaling up to MixQ
     */
    public async approveMix(
        bAsset: t.MockERC20Instance,
        mIx: t.MixInstance,
        fullMixUnits: number | BN | string,
        ...
        const approvalAmount: BN = inputIsBaseUnits
            ? new BN(fullMixUnits)
            : simpleToExactAmount(fullMixUnits, bAssetDecimals.toNumber());
        await bAsset.approve(mIx.address, approvalAmount, { from: sender });
    ...
    public async approveMixMulti(
        ...
        const result = Promise.all(
            bAssets.map((b) => this.approveMix(b, mIx, fullMixUnits, sender)),
        );
        return result;
    }
    ...
    public async getPlatformInteraction(
        ...
        const totalSupply = await mIx.totalSupply();
        const surplus = await mIx.surplus();
        const cacheSize = await mIx.cacheSize();
