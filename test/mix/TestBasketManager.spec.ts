...
import { MixMachine, StandardAccounts, SystemMachine } from "@utils/machines";
...
contract("BasketManager", async (accounts) => {
    ...
    async function createNewBasketManager(): Promise<t.BasketManagerInstance> {
        ...
        await aaveIntegration.initialize(
            nexus.address,
            [mix, governance, basketManager.address],
    ...
    before(async () => {
        ...
        integrationDetails = await mixMachine.loadBassets();
    ...
    describe("initialize()", () => {
        describe("should fail", () => {
            ...
            it("when mIx address is zero", async () => {
                const bm = await BasketManager.new();
                await expectRevert(
                    bm.initialize(
                        nexus.address,
        ...
        describe("with valid parameters", async () => {
            it("should have initialized with events", async () => {
                ...
                // should have mIx address
                expect(mix).to.equal(await basketManager.mIx());
    ...
    describe("increaseVaultBalance()", async () => {
        it("should fail when called by other than mix contract", async () => {
            await Promise.all(
    ...
    describe("collectInterest()", async () => {
        ...
        describe("should fail", async () => {
            it("when called from other than Mix", async () => {
                await expectRevert(
            ...
            it("when contract is Pasued", async () => {
                ...
                await expectRevert(
                    basketManager.collectInterest({ from: mix }),
                    "Pausable: paused",
                );
            });
        });
        ...
        it("should have interest generated", async () => {
            ...
            await basketManager.collectInterest({ from: mix });
    ...
    describe("prepareForgeBassets()", async () => {
        ...
        it("should return ForgePropsMulti", async () => {
            // rely on integration tests from the mIx to ensure that the forge props are being passed correctly
        });
    });
