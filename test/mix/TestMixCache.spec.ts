...
contract("Mix - Cache", async (accounts) => {
    ...
    const assertBasicMint = async (
        ...
        const tx = await mIx.mint(bAsset.address, approval0);
    ...
    const assertBasicRedemption = async (
        ...
        // Get balances before
        const senderMixBalBefore = await mIx.balanceOf(sa.default);
        ...
        const tx = await mIx.redeem(bAsset.address, bAssetExact);
        ...
        const senderMixBalAfter = await mIx.balanceOf(sa.default);
    ...
    const assertSwap = async (
        ...
        const isMint = mIx.address === outputAsset.address;
        ...
        const inputBassetBefore = await mixMachine.getBasset(basketManager, inputBasset.address);
        ...
        const outputBassetBefore = isMint
            ? null
            : await mixMachine.getBasset(basketManager, outputAsset.address);
        ...
        const swapOutputResponse = await mIx.getSwapOutput(
            inputBasset.address,
            outputAsset.address,
            approval0,
            { from: sender },
        ...
        // Expect to be used in cache
        const platformInteractionIn = await mixMachine.getPlatformInteraction(
            mIx,
            "deposit",
            approval0,
        ...
        const platformInteractionOut = isMint
            ? null
            : await mixMachine.getPlatformInteraction(
                  mIx,
                  "withdrawal",
        ...
        if (isMint) {
            await expectEvent(swapTx.receipt, "Minted", {
                minter: sender,
                recipient,
                mIxQuantity: expectedOutputValue,
    ...
    context("validating simple transactions with different cache sizes", () => {
        ...
        const runTxs = async () => {
            ...
            await assertBasicRedemption(mixDetails, new BN(10), bAssets[1], true);
            await assertBasicMint(mixDetails, new BN(10), bAssets[0]);
            await assertBasicMint(mixDetails, new BN(20), bAssets[0]);
            await assertBasicMint(mixDetails, new BN(12), bAssets[1]);
            await assertBasicMint(mixDetails, new BN(3), bAssets[2]);
            await assertBasicRedemption(mixDetails, new BN(1), bAssets[1], true);
            await assertSwap(mixDetails, bAssets[0], bAssets[1], new BN(10), true, sa.dummy4);
            await assertSwap(mixDetails, bAssets[0], bAssets[1], new BN(6), true);
            await assertSwap(mixDetails, bAssets[1], bAssets[2], new BN(20), true);
            await assertBasicMint(mixDetails, new BN(3), bAssets[2]);
            await assertBasicMint(mixDetails, new BN(6), bAssets[0]);
            await assertBasicMint(mixDetails, new BN(12), bAssets[0]);
            await assertBasicRedemption(mixDetails, new BN(1), bAssets[0], true);
            await assertBasicRedemption(mixDetails, new BN(14), bAssets[0], true);
            await assertSwap(mixDetails, bAssets[2], bAssets[3], new BN(14), true);

            // Test savings deposit
            await mixDetails.mIx.approve(systemMachine.savingsContract.address, new BN(1), {
                from: sa.default,
            });
            await systemMachine.savingsContract.methods["depositSavings(uint256)"](new BN(1), {
                from: sa.default,
            });
            await assertSwap(mixDetails, bAssets[1], bAssets[2], new BN(1), true);
            await mixDetails.mIx.approve(systemMachine.savingsContract.address, new BN(1));
            await systemMachine.savingsContract.methods["depositSavings(uint256)"](new BN(1));
        };
        it("should exec with 0%", async () => {
            await mixDetails.mIx.setCacheSize(0, {
                from: sa.governor,
            });
            await runTxs();
        });
        it("should exec with 10%", async () => {
            await mixDetails.mIx.setCacheSize(simpleToExactAmount(1, 17), {
                from: sa.governor,
            });
            await runTxs();
        });
        it("should exec with 20%", async () => {
            await mixDetails.mIx.setCacheSize(simpleToExactAmount(2, 17), {
                from: sa.governor,
            });
            await runTxs();
        });
        it("should allow for changing cache sizes", async () => {
            ...
            await runTxs();
            await mixDetails.mIx.setCacheSize(simpleToExactAmount(2, 16), {
                from: sa.governor,
            });
            await runTxs();
            await mixDetails.mIx.setCacheSize(simpleToExactAmount(1, 17), {
    ...
    context("testing the fine tuning of cache limits", () => {
        ...
        beforeEach("reset", async () => {
            ...
            const { basketManager, bAssets } = mixDetails;
        ...
        it("should deposit if it goes over", async () => {
            ...
            await assertBasicMint(mixDetails, 5, bAsset.contract, false);
            // Total Supply = 105, cache = 5, vaultBalance = 30
            // second tx pushes above
            await assertBasicMint(mixDetails, 6, bAsset.contract, false);
        ...
        it("should withdrawRaw if there is cache balance", async () => {
            ...
            const bAssetDecimals = await bAsset.contract.decimals();
            // first tx should move it to 5 units in cache
            await assertBasicRedemption(mixDetails, 1, bAsset.contract, true);
            // Total Supply = 99, cache = 5, vaultBalance = 24
            // second tx withdraws raw
            await assertBasicRedemption(mixDetails, 4, bAsset.contract, true);
        ...
        it("should withdraw to mean", async () => {
            ...
            const bAsset = bAssetsBefore[0];
            const bAssetDecimals = await bAsset.contract.decimals();
            const senderBalStart = await mIx.balanceOf(sa.default);
            // first tx should move it to 5 units in cache
            await assertBasicRedemption(mixDetails, 1, bAsset.contract, true);
            // Total Supply = 99, cache = 5, vaultBalance = 24
            // second tx resets the cache and withdraws
            await assertBasicRedemption(mixDetails, 10, bAsset.contract, true);
            // Total Supply = 89, cache = 5, vaultBalance = 25
            const cacheBalAfter = await bAsset.contract.balanceOf(bAsset.integrator.address);
            expect(cacheBalAfter).bignumber.eq(simpleToExactAmount("4.95003", bAssetDecimals));
            // Sender should have 11 less mIx
            const senderBalAfter = await mIx.balanceOf(sa.default);
    ...
    context("ensuring redeemMix analyses max cache and surplus correctly", () => {
        // start with a total supply of 100 and no cache
        let bAssetsBefore: Basset[];
        beforeEach("reset", async () => {
            await runSetup(false);
        ...
        it("should reset all assets to cache mid point", async () => {
            ...
            const senderBalStart = await mIx.balanceOf(sa.default);
            // first tx should move it to 5 units in cache
            await mIx.redeemMix(simpleToExactAmount(1, 18), sa.default);
            // Total Supply = 99, cache = 5, vaultBalance = 99.0003
            // second tx resets the cache and withdraws
            await mIx.redeemMix(simpleToExactAmount(22, 18), sa.default);
        ...
        it("should accumulate a surplus over multiple tx's", async () => {
            const { mIx } = mixDetails;
            const surplusStart = await mIx.surplus();
            const totalSupplyStart = await mIx.totalSupply();
            // first tx should move it to 5 units in cache
            await mIx.redeemMix(simpleToExactAmount(1, 18), sa.default);
            // Total Supply = 99, cache = 5, vaultBalance = 99.0003
            // second tx resets the cache and withdraws
    ...
    context("testing fee collection and distribution", () => {
        before("generate some surplus and lending market interest", async () => {
            await runSetup(true);
            await mixDetails.mIx.redeemMix(simpleToExactAmount(20, 18), sa.default);
            const surplus = await mixDetails.mIx.surplus();
            expect(surplus).bignumber.eq(simpleToExactAmount("0.006", 18));
        });
        it("allows SM to collect surplus", async () => {
            const { mIx } = mixDetails;
            const compositionBefore = await mixMachine.getBasketComposition(mixDetails);
            const balBefore = await mIx.balanceOf(systemMachine.savingsContract.address);
            await mixDetails.mIx.approve(systemMachine.savingsContract.address, new BN(1), {
                from: sa.default,
            });
            await systemMachine.savingsContract.methods["depositSavings(uint256)"](new BN(1), {
            ...
            const compositionAfter = await mixMachine.getBasketComposition(mixDetails);
            ...
            const balAfter = await mIx.balanceOf(systemMachine.savingsContract.address);
            ...
            const compositionEnd = await mixMachine.getBasketComposition(mixDetails);
            ...
            const balEnd = await mIx.balanceOf(systemMachine.savingsContract.address);
            expect(balEnd).bignumber.eq(balAfter.add(new BN(1)));
        });
        it("allows SM to collect platform interest", async () => {
            const { mIx } = mixDetails;
            const compositionBefore = await mixMachine.getBasketComposition(mixDetails);
            const balBefore = await mIx.balanceOf(systemMachine.savingsManager.address);
            const sumBefore = compositionBefore.bAssets.reduce(
                (p, c, i) => p.add(applyRatio(c.actualBalance, c.ratio)),
                new BN(0),
            );

            await systemMachine.savingsManager.collectAndStreamInterest(mIx.address);

            const compositionAfter = await mixMachine.getBasketComposition(mixDetails);
            const balAfter = await mIx.balanceOf(systemMachine.savingsManager.address);
