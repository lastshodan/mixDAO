...
contract("Mix - Mint", async (accounts) => {
    ...
    const assertFailedMint = async (
        ...
        const approval: BN = await mixMachine.approveMix(bAsset, mIx, amount);
        await expectRevert(mIx.mint(bAsset.address, approval), reason);
    };

    // Helper to assert basic minting conditions, i.e. balance before and after
    const assertBasicMint = async (
        md: MixDetails,
        mIxMintAmount: BN | number,
        bAsset: t.MockERC20Instance,
        useMintTo = false,
        ...
        const recipientBalBefore = await mIx.balanceOf(derivedRecipient);
        ...
        const tx = useMintTo
            ? await mIx.mintTo(bAsset.address, approval0, derivedRecipient, { from: sender })
            : await mIx.mint(bAsset.address, approval0, { from: sender });

        const mIxQuantity = simpleToExactAmount(mIxMintAmount, 18);
        const bAssetQuantity = simpleToExactAmount(mIxMintAmount, await bAsset.decimals());
        ...
        // Recipient should have mIx quantity after
        const recipientBalAfter = await mIx.balanceOf(derivedRecipient);
        expect(recipientBalAfter).bignumber.eq(recipientBalBefore.add(mIxQuantity));
    ...
    const seedWithWeightings = async (md: MixDetails, weights: Array<BN>): Promise<void> => {
    ...
    describe("minting with a single bAsset", () => {
        context("when the weights are within the ForgeValidator limit", () => {
            ...
            context("and sending to a specific recipient", async () => {
                ...
                it("should fail if recipient is 0x0", async () => {
                    ...
                    await expectRevert(
                        mIx.mintTo(bAssets[0].address, new BN(1), ZERO_ADDRESS),
                        "Must be a valid recipient",
                    );
                });
                it("should send mUSD when recipient is a contract", async () => {
                    const { bAssets, forgeValidator } = mixDetails;
                    const recipient = forgeValidator.address;
                    await assertBasicMint(mixDetails, new BN(1), bAssets[0], true, recipient);
                });
                it("should send mUSD when the recipient is an EOA", async () => {
                    const { bAssets } = mixDetails;
                    const recipient = sa.dummy1;
                    await assertBasicMint(mixDetails, new BN(1), bAssets[1], true, recipient);
                });
            });
            context("and specifying one bAsset base unit", async () => {
                ...
                it("should mint a higher q of mIx base units when using bAsset with 12", async () => {
                    ...
                    await bAsset.approve(mIx.address, new BN(1));

                    const minterBassetBalBefore = await bAsset.balanceOf(sa.default);
                    const recipientBalBefore = await mIx.balanceOf(sa.default);

                    const tx = await mIx.mint(bAsset.address, new BN(1));
                    const expectedMix = new BN(10).pow(new BN(18).sub(decimals));
                    await expectEvent(tx.receipt, "Minted", {
                        minter: sa.default,
                    ...
                    // Recipient should have mIx quantity after
                    const recipientBalAfter = await mIx.balanceOf(sa.default);
                    expect(recipientBalAfter).bignumber.eq(recipientBalBefore.add(expectedMix));
                    // Sender should have less bAsset after
            ...
            context("and not defining recipient", async () => {
                ...
                it("should mint to sender in basic mint func", async () => {
                    const { bAssets } = mixDetails;
                    await assertBasicMint(mixDetails, new BN(1), bAssets[1], false);
                });
            });
            context("using bAssets with transfer fees", async () => {
                ...
                it("should handle tokens with transfer fees", async () => {
                    ...
                    const recipientBalBefore = await mIx.balanceOf(recipient);
                    expect(recipientBalBefore).bignumber.eq(new BN(0));
                    const mIxMintAmount = new BN(10);
                    const approval0: BN = await mixMachine.approveMix(
                        bAsset,
                        mIx,
                        new BN(mIxMintAmount),
                    ...
                    // 3.0 Do the mint
                    const tx = await mIx.mintTo(bAsset.address, approval0, recipient);
                    ...
                    // 4.0 Recipient should have mIx quantity after
                    const recipientBalAfter = await mIx.balanceOf(recipient);
                    // Assert that we minted gt 99% of the bAsset
                    assertBNSlightlyGTPercent(
                        recipientBalBefore.add(mIxQuantity),
                        recipientBalAfter,
                        "0.3",
                        true,
                    );
                ...
                it("should fail if the token charges a fee but we dont know about it", async () => {
                    ...
                    // 3.0 Do the mint
                    await expectRevert(
                        mIx.mintTo(bAsset.address, approval0, sa.default),
            ...
            context("with an affected bAsset", async () => {
                it("should fail if bAsset is broken below peg", async () => {
                    ...
                    await mixMachine.approveMix(bAsset, mIx, new BN(1));
                    await expectRevert(
                        mIx.mint(bAsset.address, new BN(1)),
            ...
            context("passing invalid arguments", async () => {
                ...
                it("should revert when 0 quantities", async () => {
                    ...
                    await mixMachine.approveMix(bAsset, mIx, new BN(1));
                    await expectRevert(
                        mIx.mint(bAsset.address, new BN(0)),
                ...
                it("should fail if sender doesn't have balance", async () => {
                    ...
                    await mixMachine.approveMix(bAsset, mIx, new BN(100), sender);
                    await expectRevert(
                        mIx.mint(bAsset.address, new BN(100), { from: sender }),
                ...
                it("should fail if sender doesn't give approval", async () => {
                    ...
                    expect(await bAsset.allowance(sender, mIx.address)).bignumber.eq(new BN(0));
                ...
                it("should fail if the bAsset does not exist", async () => {
                    ...
                    await expectRevert(
                        mIx.mint(bAsset.address, new BN(100)),
            ...
            context("pushing the weighting beyond the maximum limit", async () => {
                ...
                it("should succeed so long as we don't exceed the max weight", async () => {
                    ...
                    const bAsset = bAssets[0];
                    const approval: BN = await mixMachine.approveMix(
                        bAsset,
                        mIx,
                        new BN(2),
                    );
                    await expectRevert(
                        mIx.mint(bAsset.address, approval),
                    ...
                    await assertBasicMint(mixDetails, new BN(2), bAsset, false);
                });
            });
            it("should mint with single bAsset", async () => {
                const { bAssets, mIx } = mixDetails;
                const oneMix = simpleToExactAmount(1, 18);
                const mUSD_bal0 = await mIx.balanceOf(sa.default);
                ...
                await mIx.mint(bAssets[0].address, approval0, {
                    from: sa.default,
                });

                const mUSD_bal1 = await mIx.balanceOf(sa.default);
                expect(mUSD_bal1).bignumber.eq(mUSD_bal0.add(oneMix));

                const approval1: BN = await mixMachine.approveMix(
                    bAssets[1],
                    mIx,
                    1,
                    sa.default,
                );
                await mIx.mint(bAssets[1].address, approval1, {
                    from: sa.default,
                });

                const mUSD_bal2 = await mIx.balanceOf(sa.default);
                expect(mUSD_bal2).bignumber.eq(mUSD_bal1.add(oneMix));

                const approval2: BN = await mixMachine.approveMix(
                    bAssets[2],
                    mIx,
                    1,
                    sa.default,
                );
                await mIx.mint(bAssets[2].address, approval2, {
                    from: sa.default,
                });

                const mUSD_bal3 = await mIx.balanceOf(sa.default);
                expect(mUSD_bal3).bignumber.eq(mUSD_bal2.add(oneMix));

                const approval3: BN = await mixMachine.approveMix(
                    bAssets[3],
                    mIx,
                    1,
                    sa.default,
                );
                await mIx.mint(bAssets[3].address, approval3, {
                    from: sa.default,
                });

                const mUSD_bal4 = await mIx.balanceOf(sa.default);
                expect(mUSD_bal4).bignumber.eq(mUSD_bal3.add(oneMix));
            });
        });
        context("with a fluctuating basket", async () => {
            describe("minting when a bAsset has just been removed from the basket", async () => {
                ...
                it("should still deposit to the right lending platform", async () => {
                    ...
                    await assertBasicMint(mixDetails, new BN(1), bAssets[3], false);
                ...
                it("should not be possible to mint with the removed bAsset", async () => {
                    const { bAssets, mIx } = mixDetails;
                    await expectRevert(
                        mIx.mint(bAssets[1].address, new BN(1)),
                        "bAsset does not exist",
                    );
                });
            });
        ...
        context("when there are a large number of bAssets in the basket", async () => {
            ...
            before(async () => {
                await runSetup(false);
                const { aaveIntegration, basketManager } = mixDetails;
        ...
        context("when the basket manager returns invalid response", async () => {
            ...
            it("should mint nothing if the preparation returns invalid from manager", async () => {
                const { mIx, forgeValidator } = mixDetails;
                // mintSingle
                const bAsset = await MockERC20.new("Mock", "MKK", 18, sa.default, 1000);
                ...
                await mixMachine.approveMix(bAsset, mIx, new BN(1000));

                const bAssetBalBefore = await bAsset.balanceOf(sa.default);
                const mIxBalBefore = await mockMix.balanceOf(sa.default);
                const mIxSupplyBefore = await mockMix.totalSupply();

                // Should mint nothing due to the forge preparation being invalid
                await mockMix.mint(bAsset.address, new BN(1000));

                const bAssetBalAfter = await bAsset.balanceOf(sa.default);
                expect(bAssetBalBefore).bignumber.eq(bAssetBalAfter);
                const mIxBalAfter = await mockMix.balanceOf(sa.default);
                expect(mIxBalBefore).bignumber.eq(mIxBalAfter);
                const mIxSupplyAfter = await mockMix.totalSupply();
                expect(mIxSupplyBefore).bignumber.eq(mIxSupplyAfter);
            });
            it("should fail if given an invalid integrator", async () => {
                ...
                await expectRevert(
                    mockMix.mint(bAsset.address, new BN(100)),
                    "SafeERC20: low-level call failed",
                );
            ...
            it("reverts if the BasketManager is paused", async () => {
                ...
                await expectRevert(mIx.mint(bAsset.address, new BN(100)), "Pausable: paused");
            });
        });
        context("when the mIx has failed", () => {
            ...
            it("should revert any mints", async () => {
                ...
                const bAsset0 = bAssets[0];
                await assertFailedMint(mIx, bAsset0, new BN(1), "Basket must be alive");
            });
        });
    ...
    describe("minting with multiple bAssets", () => {
        ...
        const assertMintMulti = async (
            md: MixDetails,
            mIxMintAmounts: Array<BN | number>,
            ...
            const approvals: Array<BN> = await Promise.all(
                bAssets.map((b, i) =>
                    mixMachine.approveMix(b, mIx, new BN(mIxMintAmounts[i])),
                ),
            );
            const tx = await mIx.mintMulti(
            ...
            const mIxQuantity = simpleToExactAmount(
                mIxMintAmounts.reduce((p, c, i) => new BN(p).add(new BN(c)), new BN(0)),
                18,
            );
            const bAssetQuantities = mIxMintAmounts.map((m, i) =>
        ...
        context("when the weights are within the ForgeValidator limit", () => {
            context("and sending to a specific recipient", async () => {
                ...
                it("should mint selected bAssets only", async () => {
                    ...
                    await assertMintMulti(
                        mixDetails,
                        [5, 10],
                        [mixDetails.bAssets[2], mixDetails.bAssets[0]],
                    );
                ...
                it("should fail if recipient is 0x0", async () => {
                    const { mIx, bAssets } = mixDetails;
                    await expectRevert(
                        mIx.mintMulti([bAssets[0].address], [new BN(1)], ZERO_ADDRESS),
                        "Must be a valid recipient",
                    );
                });
                it("should send mUSD when recipient is a contract", async () => {
                    const { bAssets, forgeValidator } = mixDetails;
                    const recipient = forgeValidator.address;
                    await assertMintMulti(mixDetails, [new BN(1)], [bAssets[0]], recipient);
                });
                it("should send mUSD when the recipient is an EOA", async () => {
                    const { bAssets } = mixDetails;
                    const recipient = sa.dummy1;
            ...
            context("and specifying one bAsset base unit", async () => {
                ...
                it("should mint a higher q of mIx base units when using bAsset with 12", async () => {
                    ...
                    const tx = await mIx.mintMulti([bAsset.address], [new BN(1)], sa.default);
                    const expectedMix = new BN(10).pow(new BN(18).sub(decimals));
            ...
            context("using bAssets with transfer fees", async () => {
                ...
                it("should handle tokens with transfer fees", async () => {
                    ...
                    // 3.0 Do the mint
                    const tx = await mIx.mintMulti([bAsset.address], [approval0], recipient);
                ...
                it("should fail if the token charges a fee but we dont know about it", async () => {
                    ...
                    // 3.0 Do the mint
                    await expectRevert(
                        mIx.mintMulti([bAsset.address], [approval0], sa.default),
                        "Asset not fully transferred",
                    );
                });
            });
            context("with an affected bAsset", async () => {
                it("should fail if bAsset is broken below peg", async () => {
                    ...
                    await mixMachine.approveMix(bAsset, mIx, new BN(1));
                    await expectRevert(
                        mIx.mintMulti([newBasset.addr], [new BN(1)], sa.default),
                        "bAsset not allowed in mint",
                    );
                });
            });
            context("passing invalid arguments", async () => {
                ...
                describe("passing an incorrect bAsset array", async () => {
                    it("should error if the array is empty", async () => {
                        const { mIx } = mixDetails;
                        await expectRevert(
                            mIx.mintMulti([], [new BN(1)], sa.default),
                ...
                it("should fail if sender doesn't have balance", async () => {
                    ...
                    await mixMachine.approveMix(bAsset, mIx, new BN(100), sender);
                    await expectRevert(
                        mIx.mintMulti([bAsset.address], [new BN(100)], sa.default, {
                            from: sender,
                        }),
                        "SafeERC20: low-level call failed",
                    );
                ...
                it("should fail if the bAsset does not exist", async () => {
                    const { mIx } = mixDetails;
                    await expectRevert(
                        mIx.mintMulti([sa.dummy1], [new BN(100)], sa.default),
                        "bAsset must exist",
                    );
                });
            });
            context("pushing the weighting beyond the maximum limit", async () => {
                ...
                it("should succeed so long as we don't exceed the max weight", async () => {
                    ...
                    await expectRevert(
                        mIx.mintMulti([bAsset.address], [approval], sa.default),
                        "bAssets used in mint cannot exceed their max weight",
                    );
                    // Set sufficient weightings allowance
                    await basketManager.setBasketWeights(
                    ...
                    // Mint should pass now
                    await assertMintMulti(mixDetails, [new BN(2)], [bAsset]);
                });
            });
            describe("minting with various orders", async () => {
                ...
                it("should mint quantities relating to the order of the bAsset indexes", async () => {
                    ...
                    await mixMachine.approveMix(bAssets[0], mIx, new BN(100));
                    await mixMachine.approveMix(bAssets[1], mIx, new BN(100));
                ...
                it("should mint using multiple bAssets", async () => {
                    const { bAssets, mIx } = mixDetails;
                    // It's only possible to mint a single base unit of mIx, if the bAsset also has 18 decimals
                    // For those tokens with 12 decimals, they can at minimum mint 1*10**6 mIx base units.
                    // Thus, these basic calculations should work in whole mIx units, with specific tests for
                    // low decimal bAssets
                    ...
                    const approvals2 = await mixMachine.approveMixMulti(
                        [bAssets[0], bAssets[1], bAssets[2], bAssets[3]],
                        mIx,
                        1,
                        sa.default,
                    );
                    const mUSD_balBefore = await mIx.balanceOf(sa.default);
                    await mIx.mintMulti(
                        [
                            bAssets[0].address,
                            bAssets[1].address,
                    ...
                    const mUSD_balAfter = await mIx.balanceOf(sa.default);
        ...
        context("with a fluctuating basket", async () => {
            describe("minting when a bAsset has just been removed from the basket", async () => {
                ...
                it("should not be possible to mint with the removed bAsset", async () => {
                    const { mIx, bAssets } = mixDetails;
                    await expectRevert(
                        mIx.mintMulti([bAssets[1].address], [new BN(1)], sa.default),
                        "bAsset must exist",
                    );
                });
            });
        ...
        context("when the weights exceeds the ForgeValidator limit", async () => {
            ...
            it("should succeed if bAsset is underweight", async () => {
                ...
                expect(composition.bAssets[2].overweight).to.eq(false);
                const bAsset2 = bAssets[2];
                await mixMachine.approveMix(bAsset2, mIx, new BN(2));
                await expectRevert(
                    mIx.mintMulti(
                        [bAsset2.address],
        ...
        context("when the basket manager returns invalid response", async () => {
            ...
            it("should mint nothing if the preparation returns invalid from manager", async () => {
                ...
                // Should mint nothing due to the forge preparation being invalid
                await mockMix.mintMulti([bAsset.address], [new BN(1)], sa.default);
            ...
            it("should fail if given an invalid integrator", async () => {
                ...
                // Should revert since we can't just call an invalid integrator
                await expectRevert(
                    mockMix.mintMulti([bAsset.address], [new BN(1)], sa.default),
                    "SafeERC20: low-level call failed",
                );
            ...
            it("reverts if the BasketManager is paused", async () => {
                const { mIx, basketManager, bAssets } = mixDetails;
                await basketManager.pause({ from: sa.governor });
                expect(await basketManager.paused()).eq(true);
                await expectRevert(
                    mIx.mintMulti([bAssets[0].address], [new BN(1)], sa.default),
                    "Pausable: paused",
                );
            });
        });
        context("when the mIx has failed", () => {
            ...
            it("should revert any mints", async () => {
                ...
                await mixMachine.approveMix(bAsset0, mIx, new BN(2));
                await expectRevert(
                    mIx.mintMulti([bAsset0.address], [new BN(1)], sa.default),
