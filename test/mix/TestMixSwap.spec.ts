...
contract("Mix - Swap", async (accounts) => {
     ...
     * @param mIx Mix instance upon which to call swap
    ...
    const assertFailedSwap = async (
        ...
        await expectRevert(
            mIx.swap(inputBasset.address, outputAsset.address, approval, recipient, {
                from: sender,
            }),
            expectedReason,
        );
        ...
        if (callSwapOutput) {
            if (swapOutputRevertExpected) {
                await expectRevert(
                    mIx.getSwapOutput(inputBasset.address, outputAsset.address, approval, {
                        from: sender,
                    }),
                    expectedReason,
                );
    ...
    const assertSwap = async (
        ...
        const platformInteraction_in = await mixMachine.getPlatformInteraction(
            mIx,
            "deposit",
            approval0,
    ...
    describe("swapping assets", () => {
        context("when the weights are within the ForgeValidator limit", () => {
            context("and sending to a specific recipient", async () => {
                ...
                it("should swap two bAssets", async () => {
                    ...
                    await assertSwap(mixDetails, bAssets[0], bAssets[1], 1, true, sa.dummy1);
                });
            ...
            context("and using the mIx as the output asset", async () => {
                it("should mint mIxs instead of swapping", async () => {
                    const { bAssets, mIx } = mixDetails;
                    await assertSwap(
                        mixDetails,
                        bAssets[0],
                        await MockERC20.at(mIx.address),
                        1,
                        true,
                    );
                });
            });
            context("and using the mIx as the input asset", async () => {
            ...
            context("passing invalid arguments", async () => {
                ...
                it("should fail if sender doesn't give approval", async () => {
                    ...
                    expect(await input.allowance(sender, mIx.address)).bignumber.eq(new BN(0));
                    expect(await input.balanceOf(sender)).bignumber.eq(new BN(10000));
                    await expectRevert(
                        mIx.swap(input.address, bAssets[1].address, new BN(100), sa.default, {
                            from: sender,
                        }),
                        "SafeERC20: low-level call failed",
                    );
                ...
                it("should fail if *either* bAsset does not exist", async () => {
                    ...
                    await expectRevert(
                        mIx.swap(fakeBasset, realBasset, new BN(1), recipient),
                        "Input asset does not exist",
                    );
                    await expectRevert(
                        mIx.swap(realBasset, fakeBasset, new BN(1), recipient),
                ...
                it("should fail if *either* bAsset is ZERO", async () => {
                    ...
                    await expectRevert(
                        mIx.swap(realBasset.address, fakeBasset, new BN(1), sa.default),
                        expectedReason,
                    );
                    await expectRevert(
                        mIx.getSwapOutput(realBasset.address, fakeBasset, new BN(1)),
                        expectedReason,
                    );
                    await expectRevert(
                        mIx.swap(fakeBasset, realBasset.address, new BN(1), sa.default),
                        expectedReason,
                    );
                    await expectRevert(
                        mIx.getSwapOutput(fakeBasset, realBasset.address, new BN(1)),
                        expectedReason,
                    );
                });
            ...
            context("with an affected bAsset", async () => {
                ...
                it("should fail if input basset status is not Normal", async () => {
                    ...
                    await assertFailedSwap(mIx, input, output, 1, "bAsset not allowed in swap");
                });
            ...
            context("pushing the weighting beyond the maximum limit", async () => {
                ...
                it("should succeed so long as the input doesn't exceed max", async () => {
                    ...
                    await assertSwap(mixDetails, input, output, 1, true);
                });
            });
        });
        context("with a fluctuating basket", async () => {
            describe("swapping when a bAsset has just been removed from the basket", async () => {
                ...
                it("should still deposit to the right lending platform", async () => {
                    ...
                    await assertSwap(mixDetails, input, output, 1, true, sa.default);
        ...
        context("when there are a large number of bAssets in the basket", async () => {
            ...
            it("should still perform with 10 bAssets in the basket", async () => {
                ...
                mixDetails.bAssets = onChainBassets.map((o) => o.contract);
                ...
                const approvals = await Promise.all(
                    onChainBassets
                        .slice(1)
                        .map((b, i) =>
                            mixMachine.approveMix(b.contract, mIx, new BN(8), sa.default),
                        ),
                );
                await mIx.mintMulti(
                ...
                for (let i = 1; i < onChainBassets.length - 1; i += 1) {
                    ...
                    await mIx.swap(input.address, output.address, approval0, sa.default);
                }
            });
        });
        context("when the mIx is undergoing change", () => {
            beforeEach(async () => {
                await runSetup(false);
            });
            describe("when basket has failed", async () => {
                it("should throw", async () => {
                    ...
                    const input = bAssets[0];
                    const output = bAssets[1];
                    await assertFailedSwap(mIx, input, output, 1, "Basket is undergoing change");
                });
            });
