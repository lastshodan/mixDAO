...
contract("InitializableModule", async (accounts) => {
    ...
    describe("should succeed", async () => {
        ...
        it("and return bildToken address", async () => {
            const bildToken = await ctx.module.bildToken();
            expect(bildToken).to.not.equal(ZERO_ADDRESS);
            const nexusBildToken = await nexus.getModule(web3.utils.keccak256("BildToken"));
            expect(nexusBildToken).to.equal(bildToken);
        });
