...
const MockMix1 = artifacts.require("MockMix1");
...
contract("SavingsManager", async (accounts) => {
    ...
    describe("adding a SavingsContract", async () => {
        let mockMix: t.MockERC20Instance;
        const mockSavingsContract = sa.dummy4;

        before(async () => {
            mockMix = await MockMix.new("MOCK", "MOCK", 18, sa.default, new BN(10000));
        });

        it("should fail when not called by governor", async () => {
            await expectRevert(
                savingsManager.addSavingsContract(mockMix.address, mockSavingsContract, {
                    from: sa.other,
                }),
        ...
        it("should fail when savingsContract address is zero", async () => {
            await expectRevert(
                savingsManager.addSavingsContract(mockMix.address, ZERO_ADDRESS, {
                    from: sa.governor,
                }),
                "Must be valid address",
            );
        ...
        it("should fail when mIx entry already exist", async () => {
            await expectRevert(
        ...
        it("should succeed with valid parameter", async () => {
            ...
            savingsContractAddr = await savingsManager.savingsContracts(mockMix.address);
    ...
    describe("collecting and distributing Interest", async () => {
        ...
        context("with invalid arguments", async () => {
            it("should fail when mIx not exist", async () => {
                await expectRevert(
        ...
        context("with a broken mIx", async () => {
            it("fails if the mIx does not send required mIx", async () => {
                const mUSD2 = await MockMix1.new("mUSD", "mUSD", 18, sa.default, INITIAL_MINT);
        ...
        context("testing the collection and streaming of mIx interest", async () => {
            // Initial supply of 10m units
            const initialSupply = new BN(10000000);
            const liquidated1 = simpleToExactAmount(100, 18);
