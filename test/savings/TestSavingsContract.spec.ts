...
import { StandardAccounts, SystemMachine, MixDetails } from "@utils/machines";
...
const MockMix = artifacts.require("MockMix");
...
const getData = async (contract: t.SavingsContractInstance, user: string): Promise<Data> => {
    const mIx = await MockERC20.at(await contract.underlying());
    ...
    return {
        balances: {
            ...
            user: await mIx.balanceOf(user),
            contract: await mIx.balanceOf(contract.address),
        },
...
contract("SavingsContract", async (accounts) => {
    ...
    const createNewSavingsContract = async (useMockSavingsManager = false): Promise<void> => {
        ...
        // Use a mock mIx so we can dictate the interest generated
        mix = await MockMix.new("MOCK", "MOCK", 18, sa.default, 1000000000);
        ...
        const data: string = impl.contract.methods
            .initialize(nexus.address, sa.default, mix.address, "Savings Credit", "imUSD")
            .encodeABI();
    ...
    describe("constructor", async () => {
        it("should fail when mix address is zero", async () => {
            savingsContract = await SavingsContract.new();
            await expectRevert(
        ...
        it("should succeed and set valid parameters", async () => {
            ...
            expect(mix.address).to.equal(underlyingAddr);
    ...
    describe("depositing interest", async () => {
        ...
        beforeEach(async () => {
            ...
            await mix.transfer(savingsManagerAccount, simpleToExactAmount(20, 18));
            await mix.approve(savingsContract.address, simpleToExactAmount(20, 18), {
        ...
        it("should deposit interest when some credits exist", async () => {
            ...
            await mix.approve(savingsContract.address, deposit);
            await savingsContract.preDeposit(deposit, sa.default);

            const balanceBefore = await mix.balanceOf(savingsContract.address);
    ...
    describe("depositing savings", async () => {
        context("using preDeposit", async () => {
            before(async () => {
                await createNewSavingsContract();
                await mix.approve(savingsContract.address, simpleToExactAmount(1, 21));
                // This amount should not be collected
                await mix.setAmountForCollectInterest(simpleToExactAmount(100, 18));
            });
            afterEach(async () => {
                const data = await getData(savingsContract, alice);
                expect(exchangeRateHolds(data), "Exchange rate must hold");
        ...
        context("using depositSavings", async () => {
            ...
            it("should fail if the user has no balance", async () => {
                // Approve first
                await mix.approve(savingsContract.address, simpleToExactAmount(1, 18), {
                    from: sa.dummy1,
                });
            ...
            it("should deposit the mUSD and assign credits to the saver", async () => {
                ...
                await mix.approve(savingsContract.address, depositAmount, {
                    from: sa.default,
                });
                // 2. Deposit the mUSD
            ...
            context("when there is some interest to collect from the manager", async () => {
                ...
                it("should collect the interest and update the exchange rate before issuance", async () => {
                    ...
                    // Set up the mIx with some interest
                    await mix.setAmountForCollectInterest(interest);
                    await time.increase(ONE_DAY);

                    // Bob deposits into the contract
                    await mix.transfer(bob, deposit);
                    await mix.approve(savingsContract.address, deposit, { from: bob });
    ...
    describe("checking the view methods", () => {
        ...
        before(async () => {
            ...
            await mix.setAmountForCollectInterest(simpleToExactAmount(10, 18));
    ...
    describe("redeeming", async () => {
        ...
        context("using redeemCredits", async () => {
            ...
            // test the balance calcs here.. credit to mix, and public calcs
        ...
        context("with a connector that surpasses limit", async () => {
            ...
            before(async () => {
                await createNewSavingsContract();
                const connector = await MockConnector.new(savingsContract.address, mix.address);
        ...
        context("using redeem (depcrecated)", async () => {
            beforeEach(async () => {
                await createNewSavingsContract();
                await mix.approve(savingsContract.address, simpleToExactAmount(10, 18));
                await savingsContract.methods["depositSavings(uint256)"](
                    simpleToExactAmount(1, 18),
                );
            });
    ...
    describe("using the helper to check balance and redeem", async () => {
        ...
        it("should deposit and withdraw", async () => {
            ...
            // Set up the mIx with some interest
            await mix.setAmountForCollectInterest(simpleToExactAmount(5, 18));
            await mix.transfer(sa.dummy2, deposit);
            await mix.approve(savingsContract.address, deposit, { from: sa.dummy2 });
            await savingsContract.methods["depositSavings(uint256)"](deposit, {
                from: sa.dummy2,
            });

            const redeemInput = await helper.getSaveRedeemInput(savingsContract.address, deposit);
            const balBefore = await mix.balanceOf(sa.default);
            await savingsContract.redeem(redeemInput);

            const balAfter = await mix.balanceOf(sa.default);
    ...
    describe("setting connector", () => {
        ...
        it("updates the connector address, moving assets to new connector", async () => {
            ...
            const newConnector = await MockConnector.new(savingsContract.address, mix.address);
    ...
    describe("poking", () => {
        ...
        context("with a lending market connector", () => {
            ...
            before(async () => {
                await createNewSavingsContract();
                connector = await MockLendingConnector.new(savingsContract.address, mix.address);
                // Give mock some extra assets to allow inflation
                await mix.transfer(connector.address, simpleToExactAmount(100, 18));
            ...
            it("should deposit to the connector if total supply increases", async () => {
                await mix.approve(savingsContract.address, simpleToExactAmount(1, 20));
                await savingsContract.methods["depositSavings(uint256)"](deposit);
        ...
        context("with a vault connector", () => {
            ...
            before(async () => {
                await createNewSavingsContract();
                connector = await MockVaultConnector.new(savingsContract.address, mix.address);
        ...
        context("with no connector", () => {
            ...
            it("simply updates the exchangeRate using the raw balance", async () => {
                ...
                await mix.transfer(savingsContract.address, airdrop);
    ...
    describe("testing emergency stop", () => {
        ...
        before(async () => {
            ...
            await mix.transfer(bob, simpleToExactAmount(100, 18));
            await mix.approve(savingsContract.address, simpleToExactAmount(1, 21));
            await savingsContract.preDeposit(deposit, alice);
    ...
    context("performing multiple operations from multiple addresses in sequence", async () => {
        ...
        it("should give existing savers the benefit of the increased exchange rate", async () => {
            ...
            await mix.transfer(saver2, saver2deposit);
            await mix.transfer(saver3, saver3deposit);
            await mix.transfer(saver4, saver4deposit);
            await mix.approve(savingsContract.address, MAX_UINT256, { from: saver1 });
            await mix.approve(savingsContract.address, MAX_UINT256, { from: saver2 });
            await mix.approve(savingsContract.address, MAX_UINT256, { from: saver3 });
            await mix.approve(savingsContract.address, MAX_UINT256, { from: saver4 });
            ...
            await mix.setAmountForCollectInterest(interestToReceive1);
            await time.increase(ONE_DAY);
            ...
            await mix.setAmountForCollectInterest(interestToReceive2);
            await time.increase(ONE_DAY);
            ...
            await mix.setAmountForCollectInterest(interestToReceive3);
            await time.increase(ONE_DAY);
            ...
            await mix.setAmountForCollectInterest(interestToReceive4);
            await time.increase(ONE_DAY);
